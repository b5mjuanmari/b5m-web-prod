...
def moztu(gpkg_azpikoa, gpkg_gainekoa, gpkg_irteera, log):
    ...
    # --- 6. Intersekzioa zatika ---
    log.info("Intersekzioa zatika...")
    t0 = time.perf_counter()
    berria = []
    ...
    log.info(f"Intersekzioa: {formatu_denbora(t_int)}")
    ...
    log.info(
        ...
        f"({aurretik - ondoren} ezabatu dira gainekoarekin talkarik "
        f"ez zutelako)."
    )

    if gdf_out.empty:
        log.abisua(
            "Emaitza hutsik dago; gainekoak ez du azpikoarekin "
            "talkarik egiten."
        )
    ...
    log.info(
        f"Mozketa: {len(gdf_out)} elementu, eremuak: "
...
def main():
    ...
    try:
        moztu(gpkg_azpikoa, gpkg_gainekoa, gpkg_irteera, log)
